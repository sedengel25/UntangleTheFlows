...
#         if version:
#             used_packages[name] = version
#     except ImportError:
#         continue
# 
# with open("python-used-packages.txt", "w") as f:
...
parser = argparse.ArgumentParser(description="Load input data and execute PaCMAP embedding.")
parser.add_argument("--path_file_directory", type=str, required=True, help="Directory containing the matrix and coordinates file.")
parser.add_argument("--i", type=int, required=True)
args = parser.parse_args()


path_file_directory = os.path.abspath(args.path_file_directory)  
path_distmat = os.path.join(path_file_directory, "dist_mat.npy")

path_df_coordinates = os.path.join(path_file_directory, "df_coordinates.csv")

distmat = np.load(path_distmat)
print(distmat[0:5,0:5])
print(distmat.shape, flush=True)
int_i = args.i

# Initialisierungsdaten laden
def load_init_data(coordinates_path):
    try:
        df_coordinates = pd.read_csv(coordinates_path)
        print(f"CSV-Datei erfolgreich geladen: {coordinates_path}")

        # In ein NumPy-Array konvertieren
        init_data = df_coordinates.to_numpy(dtype=np.float32)
        print(f"Initialisierungsdaten erfolgreich in NumPy-Array konvertiert. Shape: {init_data.shape}")
        return init_data
    except Exception as e:
        print(f"Fehler beim Laden der CSV-Datei: {e}")
        return None

init_data = load_init_data(path_df_coordinates)
print(init_data.shape, flush=True)
if init_data is None:
    raise ValueError(f"Initialisierungsdaten konnten nicht geladen werden aus {path_df_coordinates}.")
...
def calculate_dist(x1, x2, distance_index):
    i_ind = int(min(x1[0], x2[0]))
    j_ind = int(max(x1[0], x2[0]))
    ...
    elif distance_index == 4:
        return distmat[i_ind, j_ind]
...
def generate_pair(
    ...
    for i in range(n):
        nbrs_ = np.argpartition(distmat[i], n_neighbors_extra + 1)[:(n_neighbors_extra + 1)] # nbrs_ = tree.get_nns_by_item(i, n_neighbors_extra + 1)
        nbrs_ = nbrs_[np.argsort(distmat[i][nbrs_])] # nbrs[i, :] = nbrs_[1:]
        nbrs[i, :] = nbrs_[1:]
        for j in range(n_neighbors_extra):
            #print(f"i = {i}, nbrs_ = {nbrs_}")
            #print(f"nbrs_[j] = {nbrs_[j]}, j = {j}, n_neighbors_extra = {n_neighbors_extra}")
            #print(f"distmat.shape = {distmat.shape}, i = {i}, j = {nbrs_[j]}")
            knn_distances[i, j] = calculate_dist(np.array([i], np.float32), np.array([nbrs_[j]], np.float32), option) 
            #print(f"kNN-Distanz für i={i}, j={j} (Nachbar {nbrs_[j]}): {knn_distances[i, j]}")# knn_distances[i, j] = tree.get_distance(i, nbrs[i, j])
    print_verbose("Found nearest neighbor", verbose)
...
class PaCMAP(BaseEstimator):
    ...
    def __init__(self,
                 ...
                 distance="euclidean",
                 lr=1.0,
                 num_iters=(100, 100, 250),#100,100,250
                 verbose=False,
                 apply_pca=True,
                 intermediate=False,
...
if __name__ == "__main__":
    ...
    embedding = pacmap_instance.fit_transform(distmat, init=init_data)
    embedding_file = os.path.join(path_file_directory, 
    f"embedding_4d_{int_i}.npy")
